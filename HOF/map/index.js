...
const words = ["apple", "banana", "cherry","sarfaraz"];

const upperCase = words.map((word,i)=>word.toUpperCase());

// Given an array of numbers, use the map method to return a new array with each number 
// squared.
const square = arr.map((item,i)=>item*item);


/*
Write a program that takes an array of strings and uses map to return a new array with the lengths of 
each string. */

const lengthOfString = words.map((str,i)=>{
   return str.length
})
console.log("lengthOfString",lengthOfString)
/* 
Given an array of objects representing people with name properties, use map to create an array of just the names.
*/
...
const productName =products.map((product,i)=>{
   return product.title
})
console.log("productName",productName);

/*
Create a function that takes an array of temperatures in Celsius and uses map to 
return an array of the same temperatures in Fahrenheit (F = C * 9/5 + 32). */

const temperatures = [35,40,36,36,44,42,41,46,48,48];

const tempInFahrenheit = temperatures.map((temp,i)=>{
    return ( temp * 9/5 + 32)
})
